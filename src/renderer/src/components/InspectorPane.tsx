...
interface InspectorPaneProps {
  ...
  onUpdateEntity: (id: string, data: any) => void
...
export default function InspectorPane({ project, entities, onAddEntity, onUpdateEntity, onDeleteEntity }: InspectorPaneProps) {
      ...
      <aside class="ih-pane ih-inspector">
        ...
        <div class="p-3 space-y-2 text-xs">
          {filtered.map(entity => (
            <div
              key={entity.id}
              class="ih-entity-card hover:border-[var(--accent)]"
      ...
      {selectedEntity && (
        <EntityDetailModal
          ...
          onUpdate={async (id, data) => {
            const updated = await window.hub.updateEntity(id, data)
            onUpdateEntity(id, updated)
          }}
          onDelete={async (id) => {
            await window.hub.softDeleteEntity(id)
            onDeleteEntity(id)
